...
# ---------------------------------------------------------------------------
# Duong dan goc cac module (suy ra tu vi tri file nay)
# ---------------------------------------------------------------------------
THIS_DIR = Path(__file__).resolve().parent       # .../Python/EaF/MODULE_AUTO
WORKSPACE_ROOT = THIS_DIR.parent                 # .../Python/EaF  (chua cac MODULE_*)
DATA_ROOT = WORKSPACE_ROOT.parent                # .../Python      (chua CSV, Filepcap)
...
def process_pcap(pcap_path: str) -> bool:
    ...
    try:
        ...
        _run(PYTHON_CMD + [str(EXTRACTOR), str(pcap)], "extractor")
        ...
        for i, filter_name in enumerate(FILTERS, 1):
            ...
            if not filter_path.is_file():
                logger.warning("  [%d/%d] Bo qua (khong ton tai): %s",
                               i, len(FILTERS), filter_name)
                failed_filters.append(filter_name)
                continue
            try:
                logger.info("  --- [%d/%d] %s ---", i, len(FILTERS), filter_name)
                _run(PYTHON_CMD + [str(filter_path), str(dos_features_csv)],
                     filter_name)
