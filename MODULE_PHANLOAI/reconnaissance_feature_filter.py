...
# -*- coding: utf-8 -*-
"""
reconnaissance_feature_filter.py — Module lọc đặc trưng chuyên biệt cho phát hiện tấn công Reconnaissance.

Nhận file CSV chứa toàn bộ đặc trưng mạng (40+ cột), loại bỏ các trường nhiễu,
chỉ giữ lại các trường chuyên biệt dùng để phát hiện tấn công Reconnaissance dựa trên
nghiên cứu chuẩn, sau đó xuất ra file CSV mới tối ưu hơn.

Cách sử dụng:
    python reconnaissance_feature_filter.py input.csv
    python reconnaissance_feature_filter.py input.csv -o output.csv
"""
...
# ---------------------------------------------------------------------------
# Tập đặc trưng chuyên biệt cho phát hiện tấn công Reconnaissance:
# proto, state, dpkts, dbytes, sttl, sinpkt, swin, stcpb, synack, ackdat,
# dmean, ct_srv_src, ct_dst_ltm, ct_dst_src_ltm, is_ftp_login, ct_ftp_cmd,
...
KEEP_COLUMNS: list[str] = [
    ...
    # Nhóm 1: TTL
    "sttl",
    # Nhóm 2: Lưu lượng & kích thước gói
    "dpkts", "dbytes", "dmean",
    # Nhóm 3: Timing & inter-packet (float)
    "sinpkt", "synack", "ackdat",
    # Nhóm 4: TCP window & sequence
    "swin", "stcpb",
    # Nhóm 5: Connection Tracking
    "ct_srv_src", "ct_dst_ltm", "ct_dst_src_ltm", "ct_src_ltm",
    # Nhóm 6: FTP
    "is_ftp_login", "ct_ftp_cmd",
    # Nhóm 7: Cùng IP/Port nguồn-đích
    "is_sm_ips_ports",
    # Nhóm 8: Categorical
    "proto", "state",
]
...
CATEGORICAL_COLUMNS: list[str] = ["proto", "state"]

# Các cột nên ép kiểu số nguyên nhỏ gọn.
# Lưu ý: sinpkt, synack, ackdat là float -> KHÔNG downcast.
...
def filter_reconnaissance_features(input_path: str, output_path: str) -> pd.DataFrame:
    """
    Đọc file CSV gốc, lọc giữ lại các cột Reconnaissance chuyên biệt, tối ưu bộ nhớ
    ...
    # Tự động phát hiện dấu phân cách (, hoặc ; ...) bằng csv.Sniffer
    sep = ","
    try:
        import csv
        ...
        # Fallback: so sánh số lượng ; và , trên dòng header
        try:
            with open(input_file, "r", encoding="utf-8", errors="replace") as fh:
                header = fh.readline()
            ...
            sep = ","
    logger.info("Dấu phân cách phát hiện: %r", sep)
    df = pd.read_csv(input_file, low_memory=False, sep=sep)
    ...
    if name_part.endswith("_reconnaissance_features"):
        base_name = name_part[:-24]
...
DEFAULT_OUTPUT_DIR = r"D:\1LearnandStudy\Program_Language\Python\CSV\Filter_Reconnaissance_feature"
...
def process_directory(input_dir: str, output_dir: str) -> None:
    ...
    filter_reconnaissance_features cho từng file. Kết quả được lưu vào output_dir
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if name_part.endswith("_raw"):
            base_name = name_part[:-4]
        elif name_part.endswith("_reconnaissance_features"):
            base_name = name_part[:-24]
        else:
            base_name = name_part
        output_file = out_path / f"{base_name}_reconnaissance_features.csv"
        logger.info("")
        logger.info("─── [%d/%d] %s ───", i, len(csv_files), csv_file.name)
        filter_reconnaissance_features(str(csv_file), str(output_file))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Lọc đặc trưng chuyên biệt cho phát hiện tấn công Reconnaissance. "
    ...
    if input_path.is_dir():
        ...
        if args.output:
            ...
            if name_part.endswith("_raw"):
                ...
                base_name = name_part[:-25]
            else:
                base_name = name_part
            output_path = str(dir_name / f"{base_name}_reconnaissance_features.csv")
        filter_reconnaissance_features(str(input_path), output_path)
    else:
