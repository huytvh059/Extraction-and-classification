...
# -*- coding: utf-8 -*-
"""
generic_feature_filter.py — Module lọc đặc trưng tổng quát (generic) cho phân loại lưu lượng mạng.

Nhận file CSV chứa toàn bộ đặc trưng mạng (40+ cột), loại bỏ các trường nhiễu,
chỉ giữ lại tập đặc trưng generic chuẩn, sau đó xuất ra file CSV mới tối ưu hơn.

Tập đặc trưng giữ lại:
    proto, state, service, dsport, dur, spkts, dpkts, sbytes, dbytes,
    sttl, dttl, ct_src_ltm, ct_srv_src, ct_dst_src_ltm, ct_state_ttl,
    trans_depth, is_ftp_login, ct_ftp_cmd, payload (Payload mã hóa)

Cách sử dụng:
    python generic_feature_filter.py input.csv
    python generic_feature_filter.py input.csv -o output.csv
"""
...
# ---------------------------------------------------------------------------
# Danh sách các cột cần giữ lại — tập đặc trưng generic
# ---------------------------------------------------------------------------
# Tên cột "payload" đại diện cho trường "Payload mã hóa" (encoded payload).
# Một số tập dữ liệu dùng tên khác (vd: payload_bytes, payload_hex) -> xem
# PAYLOAD_ALIASES bên dưới để tự dò.
KEEP_COLUMNS: list[str] = [
    ...
    # Nhóm 3: Thời lượng
    "dur",
    # Nhóm 4: Số gói / kích thước
    "spkts", "dpkts", "sbytes", "dbytes",
    # Nhóm 5: TTL & Trạng thái
    "sttl", "dttl", "ct_state_ttl",
    # Nhóm 6: Connection Tracking
    "ct_src_ltm", "ct_srv_src", "ct_dst_src_ltm",
    # Nhóm 7: HTTP / FTP
    "trans_depth", "is_ftp_login", "ct_ftp_cmd",
    # Nhóm 8: Payload mã hóa
    "payload",
]

# Các tên cột có thể tương ứng với "Payload mã hóa".
PAYLOAD_ALIASES: list[str] = [
    "payload", "payload_encoded", "payload_bytes", "payload_hex",
    "encoded_payload", "payload_data", "raw_payload",
]
...
# Lưu ý: dur là float -> KHÔNG downcast.
...
def _resolve_payload_column(df: pd.DataFrame) -> None:
    """Dò cột payload theo các alias phổ biến và đổi tên về 'payload'."""
    if "payload" in df.columns:
        return
    for alias in PAYLOAD_ALIASES:
        if alias in df.columns:
            df.rename(columns={alias: "payload"}, inplace=True)
            logger.info("Đã ánh xạ cột '%s' -> 'payload' (Payload mã hóa).", alias)
            return
...
def filter_generic_features(input_path: str, output_path: str) -> pd.DataFrame:
    """
    Đọc file CSV gốc, lọc giữ lại tập đặc trưng generic, tối ưu bộ nhớ
    ...
    # Dò và đổi tên cột payload (Payload mã hóa) nếu dùng alias khác
    _resolve_payload_column(df)
    ...
    if name_part.endswith("_generic_features"):
        base_name = name_part[:-17]
...
DEFAULT_OUTPUT_DIR = r"D:\1LearnandStudy\Program_Language\Python\CSV\Filter_Generic_feature"
...
def process_directory(input_dir: str, output_dir: str) -> None:
    ...
    filter_generic_features cho từng file. Kết quả được lưu vào output_dir
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if name_part.endswith("_raw"):
            base_name = name_part[:-4]
        elif name_part.endswith("_generic_features"):
            base_name = name_part[:-17]
        else:
            base_name = name_part
        output_file = out_path / f"{base_name}_generic_features.csv"
        logger.info("")
        logger.info("─── [%d/%d] %s ───", i, len(csv_files), csv_file.name)
        filter_generic_features(str(csv_file), str(output_file))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Lọc tập đặc trưng generic cho phân loại lưu lượng mạng. "
    ...
    if input_path.is_dir():
        ...
        if args.output:
            ...
            if name_part.endswith("_raw"):
                ...
                base_name = name_part
            output_path = str(dir_name / f"{base_name}_generic_features.csv")
        filter_generic_features(str(input_path), output_path)
    else:
