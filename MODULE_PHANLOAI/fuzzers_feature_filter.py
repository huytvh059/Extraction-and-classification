...
# -*- coding: utf-8 -*-
"""
fuzzers_feature_filter.py — Module lọc đặc trưng chuyên biệt cho phát hiện tấn công Fuzzers.

Nhận file CSV chứa toàn bộ đặc trưng mạng (40+ cột), loại bỏ các trường nhiễu,
chỉ giữ lại các trường chuyên biệt dùng để phát hiện tấn công Fuzzers dựa trên
nghiên cứu chuẩn, sau đó xuất ra file CSV mới tối ưu hơn.

Cách sử dụng:
    python fuzzers_feature_filter.py input.csv
    python fuzzers_feature_filter.py input.csv -o output.csv
"""
...
# ---------------------------------------------------------------------------
# Tập đặc trưng chuyên biệt cho phát hiện tấn công Fuzzers.
KEEP_COLUMNS: list[str] = [
    ...
    "rate", "sload", "sbytes", "dbytes", "dpkts", "smean", "dmean",
    # Nhóm 3: TTL & Trạng thái
    "sttl", "dttl", "ct_state_ttl",
    # Nhóm 4: Jitter & Cửa sổ TCP
    "sjit", "swin",
    # Nhóm 5: TCP Timing
    "synack", "tcprtt",
    # Nhóm 6: Mất gói
    "sloss", "dloss",
    # Nhóm 7: Connection Tracking
    "ct_srv_src", "ct_srv_dst", "ct_dst_src_ltm",
    "ct_src_dport_ltm", "ct_dst_sport_ltm",
    # Nhóm 8: HTTP đặc trưng & cờ nhận diện
...
# Lưu ý: rate, sload, sjit, synack, tcprtt là float -> KHÔNG downcast.
INTEGER_COLUMNS: list[str] = [
    "sport", "dport",
    "sbytes", "dbytes", "dpkts", "smean", "dmean",
    "sttl", "dttl", "ct_state_ttl",
    "swin",
    "sloss", "dloss",
    "ct_srv_src", "ct_srv_dst", "ct_dst_src_ltm",
...
def filter_fuzzers_features(input_path: str, output_path: str) -> pd.DataFrame:
    """
    Đọc file CSV gốc, lọc giữ lại các cột Fuzzers chuyên biệt, tối ưu bộ nhớ
    ...
    # Tự động nhận diện dấu phân tách (',' hoặc ';')
    ...
    if name_part.endswith("_fuzzers_features"):
...
DEFAULT_OUTPUT_DIR = r"D:\1LearnandStudy\Program_Language\Python\CSV\Filter_Fuzzers_feature"
...
def process_directory(input_dir: str, output_dir: str) -> None:
    ...
    filter_fuzzers_features cho từng file. Kết quả được lưu vào output_dir
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if name_part.endswith("_raw"):
            base_name = name_part[:-4]
        elif name_part.endswith("_fuzzers_features"):
            base_name = name_part[:-17]
        else:
            base_name = name_part
        output_file = out_path / f"{base_name}_fuzzers_features.csv"
        logger.info("")
        logger.info("─── [%d/%d] %s ───", i, len(csv_files), csv_file.name)
        filter_fuzzers_features(str(csv_file), str(output_file))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Lọc đặc trưng chuyên biệt cho phát hiện tấn công Fuzzers. "
    ...
    if input_path.is_dir():
        ...
        if args.output:
            ...
            if name_part.endswith("_raw"):
                ...
                base_name = name_part
            output_path = str(dir_name / f"{base_name}_fuzzers_features.csv")
        filter_fuzzers_features(str(input_path), output_path)
    else:
