...
# -*- coding: utf-8 -*-
"""
analysis_feature_filter.py — Module lọc đặc trưng chuyên biệt cho phát hiện tấn công Analysis.

Nhận file CSV chứa toàn bộ đặc trưng mạng (40+ cột), loại bỏ các trường nhiễu,
chỉ giữ lại các trường chuyên biệt dùng để phát hiện tấn công Analysis,
sau đó xuất ra file CSV mới tối ưu hơn.

Cách sử dụng:
    python analysis_feature_filter.py input.csv
    python analysis_feature_filter.py input.csv -o output.csv
"""
...
# ---------------------------------------------------------------------------
# Tập đặc trưng chuyên biệt cho phát hiện tấn công Analysis.
KEEP_COLUMNS: list[str] = [
    ...
    # Nhóm 7: TCP
    "dtcpb", "synack",
    # Nhóm 8: Connection Tracking
    "ct_src_dport_ltm", "ct_dst_sport_ltm", "ct_dst_src_ltm",
    # Nhóm 9: Dịch vụ ứng dụng
...
CATEGORICAL_COLUMNS: list[str] = ["proto"]

# Các cột nên ép kiểu số nguyên nhỏ gọn.
# Lưu ý: dur, rate, sload, dload, sjit, synack là float -> KHÔNG downcast.
...
def filter_analysis_features(input_path: str, output_path: str) -> pd.DataFrame:
    """
    Đọc file CSV gốc, lọc giữ lại các cột Analysis chuyên biệt, tối ưu bộ nhớ
    ...
    if name_part.endswith("_analysis_features"):
        base_name = name_part[:-18]
...
DEFAULT_OUTPUT_DIR = r"D:\1LearnandStudy\Program_Language\Python\CSV\Filter_Analysis_feature"
...
def process_directory(input_dir: str, output_dir: str) -> None:
    ...
    filter_analysis_features cho từng file. Kết quả được lưu vào output_dir
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if name_part.endswith("_raw"):
            base_name = name_part[:-4]
        elif name_part.endswith("_analysis_features"):
            base_name = name_part[:-18]
        else:
            base_name = name_part
        output_file = out_path / f"{base_name}_analysis_features.csv"
        logger.info("")
        logger.info("─── [%d/%d] %s ───", i, len(csv_files), csv_file.name)
        filter_analysis_features(str(csv_file), str(output_file))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Lọc đặc trưng chuyên biệt cho phát hiện tấn công Analysis. "
    ...
    if input_path.is_dir():
        ...
        if args.output:
            ...
            if name_part.endswith("_raw"):
                ...
                base_name = name_part
            output_path = str(dir_name / f"{base_name}_analysis_features.csv")
        filter_analysis_features(str(input_path), output_path)
    else:
