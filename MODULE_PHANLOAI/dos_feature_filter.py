...
# ---------------------------------------------------------------------------
# Tập đặc trưng cuối = K&S (19) ∪ Al-Daweri (thêm dur/dttl/djit/ackdat).
# Đã BỎ sload, dload (không thuộc 2 nguồn) và spkts, dpkts (loại khỏi K&S theo
# đúng danh sách 19 trường của Kasongo & Sun).
KEEP_COLUMNS: list[str] = [
    # Nhóm định danh (Metadata — phục vụ truy vết)
    "src_mac", "dst_mac", "srcip", "dstip", "sport", "dport", "ltime",
    # Nhóm 1: TTL & Trạng thái (K&S: sttl, ct_state_ttl | Al-D: dttl)
    "sttl", "ct_state_ttl", "dttl",
    # Nhóm 2: Lưu lượng & kích thước gói (K&S)
    "sbytes", "dbytes", "smean", "dmean", "rate",
    # Nhóm 3: Thời lượng (Al-D)
    "dur",
    # Nhóm 4: Mất gói (K&S)
    "sloss", "dloss",
    # Nhóm 5: TCP Timing (K&S: tcprtt, synack | Al-D: synack, ackdat)
    "tcprtt", "synack", "ackdat",
    # Nhóm 6: Jitter (Al-D)
    "djit",
    # Nhóm 7: Connection Tracking (K&S)
    "ct_srv_dst", "ct_dst_src_ltm", "ct_dst_sport_ltm",
    "ct_src_dport_ltm", "ct_srv_src",
    # Nhóm 8: Categorical (K&S: proto, state, service | Al-D: proto)
    "proto", "state", "service",
]
...
# Lưu ý: dur, rate, tcprtt, synack, ackdat, djit là float -> KHÔNG downcast.
