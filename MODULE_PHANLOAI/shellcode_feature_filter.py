...
# -*- coding: utf-8 -*-
"""
shellcode_feature_filter.py — Module lọc đặc trưng chuyên biệt cho phát hiện tấn công Shellcode.

Nhận file CSV chứa toàn bộ đặc trưng mạng (40+ cột), loại bỏ các trường nhiễu,
chỉ giữ lại các trường chuyên biệt dùng để phát hiện tấn công Shellcode,
sau đó xuất ra file CSV mới tối ưu hơn.

Cách sử dụng:
    python shellcode_feature_filter.py input.csv
    python shellcode_feature_filter.py input.csv -o output.csv
"""
...
KEEP_COLUMNS: list[str] = [
    ...
    "dpkts", "dmean", "response_body_len",
    # Nhóm 3: Throughput & Timing
    "dload", "dinpkt",
    # Nhóm 4: Mất gói
    "dloss",
    # Nhóm 5: TCP đặc trưng
    "swin", "stcpb", "tcprtt", "synack",
...
CATEGORICAL_COLUMNS: list[str] = ["service", "state"]

# Các cột nên ép kiểu số nguyên nhỏ gọn.
# Lưu ý: dload, dinpkt, tcprtt, synack là float -> KHÔNG downcast.
...
def filter_shellcode_features(input_path: str, output_path: str) -> pd.DataFrame:
    """
    Đọc file CSV gốc, lọc giữ lại các cột Shellcode chuyên biệt, tối ưu bộ nhớ
    ...
    # Tự động nhận diện dấu phân cách (, hoặc ;) bằng sniffer của engine python
    ...
    if name_part.endswith("_shellcode_features"):
        base_name = name_part[:-19]
...
DEFAULT_OUTPUT_DIR = r"D:\1LearnandStudy\Program_Language\Python\CSV\Filter_Shellcode_feature"
...
def process_directory(input_dir: str, output_dir: str) -> None:
    ...
    filter_shellcode_features cho từng file. Kết quả được lưu vào output_dir
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        if name_part.endswith("_raw"):
            base_name = name_part[:-4]
        elif name_part.endswith("_shellcode_features"):
            base_name = name_part[:-19]
        else:
            base_name = name_part
        output_file = out_path / f"{base_name}_shellcode_features.csv"
        logger.info("")
        logger.info("─── [%d/%d] %s ───", i, len(csv_files), csv_file.name)
        filter_shellcode_features(str(csv_file), str(output_file))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Lọc đặc trưng chuyên biệt cho phát hiện tấn công Shellcode. "
    ...
    if input_path.is_dir():
        ...
        if args.output:
            ...
            if name_part.endswith("_raw"):
                ...
                base_name = name_part
            output_path = str(dir_name / f"{base_name}_shellcode_features.csv")
        filter_shellcode_features(str(input_path), output_path)
    else:
